<article {{ $attributes }}>
  <x-autoevaluacion.header title="Conocimiento sobre Diversidad, Equidad e Inclusión" image="img/bg_4.png">
    A continuación queremos evaluar el nivel de conocimiento que tienes sobre esta materia. Recuerda que es una
    reflexión que sobre todo te ayudará a ti a crear un liderazgo más ético e inclusivo y así crecer personal y
    profesionalmente.
  </x-autoevaluacion.header>


  <div class="question text-center">
    <p class="mt-3 text-left">(*) Obligatorio.</p>
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">7. </span> ¿Sabes qué significan las siglas <strong>DEIBJ</strong>?(*)
    </h4>
    <h4 class="font-semibold">(0 Lo desconozco | 5 Algunas letras sí | 10 Perfectamente)</h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">8. </span> ¿Sabrías explicar la diferencia entre <strong>diversidad</strong> e
      <strong>inclusión</strong> y entre
      <strong>igualdad</strong> y <strong>equidad</strong>?(*)
    </h4>
    <h4 class="font-semibold">(0 Lo desconozco | 5 más o menos | 10 Perfectamente)</h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">9. </span> ¿Sabes qué son y cuáles son la <strong>dimensiones de la
        diversidad</strong>?(*)
    </h4>
    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">10. </span> ¿Conoces la diferencia entre <strong>
        diversidad demográfica, experiencial y
        cognitiva
      </strong> y su relación con la competitividad del talento corporativo?(*)
    </h4>
    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">11. </span>¿Sabes qué son los sesgos inconscientes y cómo generan desigualdad y exclusión social?(*)
    </h4>
    <h4 class="font-semibold">(0 Lo desconozco | 5 me suena | 10 Perfectamente)</h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">12. </span>¿Sabrías decir cuáles son los principales retos para el desarrollo humano y social?(*)
    </h4>
    <h4 class="font-semibold">(0 Lo desconozco | 5 solo algún reto | 10 Perfectamente)</h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">13. </span>¿Sabrías explicar qué es un <strong>Champion de Diversidad</strong> o
      <strong>
        un Guardián de la
        Diversidad</strong>?(*)
    </h4>
    <h4 class="font-semibold">(0 Lo desconozco | 5 solo algún reto | 10 Perfectamente)</h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">14. </span>En relación a la composición de equipos y la diversidad, ¿sabrías la
      diferencia entre un <strong>"equipo"</strong>, una <strong>"tribu"</strong> y un
      <strong>"equipo-tribu"</strong>?(*)
    </h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">15. </span>¿Sabes a qué hace referencia la <strong>ruta a la inclusión</strong> o
      <strong>inclusión journey</strong>?(*)
    </h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">16. </span>¿Sabes cuál es la diferencia entre <strong>selección laboral, selección
        inclusiva e inclusión laboral</strong>?(*)
    </h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>
  {{--  --}}
  <div class="question mt-24 text-center">
    <h4 class="mt-7 text-2xl text-verde-adecco">
      <span class="p-number">17. </span>¿Sabes cuáles son las claves para crear una <strong>marca
        inclusiva</strong>?(*)
    </h4>

    <x-autoevaluacion.custom-range pretext="Lo desconozco" postext="Perfectamente" class="answer flex flex-wrap justify-center md:justify-between gap-2" />
  </div>

</article>
